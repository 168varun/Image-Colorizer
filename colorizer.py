...
def ToBW(pre, destination):
    ...
    for filename in os.listdir(pre):
        arr = cv2.imread(pre + filename)
        #narr = cv2.resize(arr, (imsize,imsize))
        narr = cv2.cvtColor(arr, cv2.COLOR_BGR2GRAY)
        cv2.imwrite(destination + "img_"+str(i)+".jpg", narr)
        i = i + 1
...
def Accuracy(og, destination):
	...
	for image in os.listdir(og):
		col = img_to_array(load_img(og + 'img_' + str(i) + '.jpg'))
		...
		plt.imshow(ls)
		plt.show()
		imsave(dn + "acc_"+str(i)+".jpg", ls)
		print("Max percent pixel error :", np.max(ls) * 100, '%')
		print("Mean percent pixel error :", np.mean(ls) * 100, '%')
		print("Median percent pixel error :", np.median(ls) * 100, '%')
		#print("\n")
		row = [np.max(ls)*100, np.mean(ls)*100, np.median(ls)*100]
...
while(t):
	...
	if event == 'Resize' and pre !='' and destination != '':
	    Resize(pre, destination)
